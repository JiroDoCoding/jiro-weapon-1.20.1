...
public class RitualCircleEntity extends Entity {
    ...
    public void tick() {
        ...
        if (!world.isClient && this.age % DAMAGE_INTERVAL == 0) {
            ...
            List<LivingEntity> targets =
                    world.getNonSpectatingEntities(LivingEntity.class,
                            new Box(
                                    cx - RADIUS, cy - 0.1, cz - RADIUS,
                                    cx + RADIUS, cy + 4.0, cz + RADIUS
                            ));
            ...
            for (LivingEntity target : targets) {
                ...
                if (target.getBoundingBox().maxY <= cy) continue;
                ...
                if (target.damage(source, DAMAGE_AMOUNT)) {
                    totalHealing += DAMAGE_AMOUNT;
                }
            }

            if (owner != null && totalHealing > 0f) {
                owner.heal(totalHealing);
            }
        }
    }

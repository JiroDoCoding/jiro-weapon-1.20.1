    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        ...
        if (world.isClient && user.isSneaking()) {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                double radius = 0.8 + world.random.nextDouble() * 0.4;
                double dx = Math.cos(angle) * radius;
                double dz = Math.sin(angle) * radius;

                world.addParticle(ParticleTypes.CRIMSON_SPORE, x + dx, y + 0.1, z + dz, 0, 0.02, 0);
                world.addParticle(ParticleTypes.ASH, x + dx * 0.7, y + 0.25, z + dz * 0.7, 0, 0.01, 0);
            }

            world.addParticle(ParticleTypes.SOUL, x, y + 0.3, z, 0, 0.03, 0);
        }
